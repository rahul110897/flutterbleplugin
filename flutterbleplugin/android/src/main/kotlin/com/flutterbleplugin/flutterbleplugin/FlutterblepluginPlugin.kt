package com.flutterbleplugin.flutterbleplugin
...
/** FlutterblepluginPlugin */
class FlutterblepluginPlugin : FlutterPlugin, MethodCallHandler, ActivityAware,
    PluginRegistry.ActivityResultListener,
    PluginRegistry.RequestPermissionsResultListener {
    companion object {
        private val TAG = FlutterblepluginPlugin::class.java.name
    ...
    override fun onAttachedToEngine(@NonNull flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
        channel = MethodChannel(flutterPluginBinding.binaryMessenger, "flutterbleplugin")
    ...
    override fun onAttachedToActivity(binding: ActivityPluginBinding) {
        ...
        adapter = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
            ...
            /*val bluetoothManager =
                activityBinding.activity.getSystemService(Context.BLUETOOTH_SERVICE) as BluetoothManager
    ...
    private fun toMap(device: BluetoothDevice, rssi: Short): Map<String, String> {
        val map = HashMap<String, String>()
        var name = device.name ?: "Unavailable"/*device.address*/

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2 && !name.contains("-LE")) {
            name += if (device.type == DEVICE_TYPE_LE) "-LE" else ""
        }
        val distance = 10.0.pow((-69 - rssi) / (10.0 * 2.0))
        map["name"] = name
        map["address"] = device.address
        map["rssiDistance"] = distance.toString()
        map["rssiValue"] = rssi.toString()
